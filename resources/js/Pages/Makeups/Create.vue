  ...
  <div>
    ...
    <div class="bg-white rounded-md shadow overflow-hidden max-w-3xl">
      ...
      <text-input v-model="form.makeup_date" :error="form.errors.makeup_date" class="pr-6 pb-8 w-full lg:w-1/2" label="Makeup Date" type="date" />
          ...
          <select-input v-model="form.status" :error="form.errors.status" class="pr-6 pb-8 w-full lg:w-1/2" label="Status" disabled>
        ...
        <div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex justify-end items-center">
          <loading-button :loading="form.processing" class="btn-indigo" type="submit">Create Makeup</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    store() {
      this.form.post(this.route('makeups.store'))
    },
  },
}
</script>
